...
from ..mappers.kledo_mapper import aggregate_financials, from_kledo_invoice, from_kledo_invoices
from ..utils.helpers import format_currency, format_markdown_table, parse_date_range, safe_get
...
async def _revenue_summary(args: dict[str, Any], client: KledoAPIClient) -> str:
    ...
    try:
        ...
        data = await client.list_invoices(
            status_id=3, date_from=date_from, date_to=date_to, per_page=100  # LUNAS / Paid
        )
        ...
        result.append(
            f"**Penjualan Bruto (Gross Sales)**: {format_currency(float(total_gross_sales))}\n"
        )
        ...
        result.append(
            f"**Average Invoice (Gross Sales)**: {format_currency(float(avg_gross_sales))}"
        )
...
async def _outstanding_receivables(args: dict[str, Any], client: KledoAPIClient) -> str:
    ...
    try:
        ...
        for status_id in statuses:
            data = await client.list_invoices(status_id=status_id, per_page=100)
            invoices = safe_get(data, "data.data", [])
            all_invoices.extend(invoices)

        # Filter by minimum amount and sort
        outstanding = [inv for inv in all_invoices if float(safe_get(inv, "due", 0)) >= min_amount]
        ...
        if 1 in by_status:
            ...
            result.append(
                format_markdown_table(
                    headers=["Invoice #", "Customer", "Date", "Outstanding"], rows=rows
                )
            )
            result.append("")
        ...
        if 2 in by_status:
            ...
            result.append(
                format_markdown_table(
                    headers=["Invoice #", "Customer", "Date", "Paid %", "Outstanding"], rows=rows
                )
            )
            result.append("")
    ...
    try:
        ...
        for inv in invoices:
            ...
            if contact_id:
                ...
                customer_data[contact_id]["gross_sales"] += Decimal(
                    str(safe_get(inv, "amount_after_tax", 0))
                )
                customer_data[contact_id]["invoice_count"] += 1

        # Sort by net sales (highest first)
        sorted_customers = sorted(
            customer_data.items(), key=lambda x: x[1]["net_sales"], reverse=True
        )[:limit]
        ...
        for i, (customer_id, cust_data) in enumerate(sorted_customers, 1):
            ...
            avg_net_sales = (
                cust_data["net_sales"] / cust_data["invoice_count"]
                if cust_data["invoice_count"] > 0
                else Decimal(0)
            )
    ...
    try:
        ...
        for inv in all_invoices:
            ...
            aging_buckets[bucket]["invoices"].append(
                {"invoice": inv, "days_old": days_old, "due": due_amount}
            )
        ...
        for bucket_name in ["0-30", "30-60", "60-90", "90+"]:
            ...
            sorted_invoices = sorted(bucket["invoices"], key=lambda x: x["days_old"], reverse=True)[
                :10
            ]
            ...
            for item in sorted_invoices:
                ...
                rows.append(
                    [inv_number, customer, str(days), format_currency(float(due), short=True)]
                )

            result.append(
                format_markdown_table(
                    headers=["Invoice #", "Customer", "Days Old", "Outstanding"], rows=rows
                )
            )
            result.append("")
        ...
        if aging_buckets["90+"]["invoices"]:
            critical = sorted(
                aging_buckets["90+"]["invoices"], key=lambda x: x["due"], reverse=True
            )[:5]
            rows = []
            ...
            result.append(
                format_markdown_table(
                    headers=["Invoice #", "Customer", "Days Old", "Amount Due"], rows=rows
                )
            )
        else:
...
async def _customer_concentration_report(args: dict[str, Any], client: KledoAPIClient) -> str:
    ...
    try:
        ...
        for i, (customer_id, cust_data) in enumerate(sorted_customers[:10], 1):
            pct_of_total = (
                (cust_data["net_sales"] / total_net_sales * 100) if total_net_sales > 0 else 0
            )
        ...
        result.append(
            format_markdown_table(
                headers=["Rank", "Customer", "Net Sales", "% of Total", "Cumulative %"], rows=rows
            )
        )
        ...
        result.append(
            f"**80% of revenue comes from**: {customers_for_80_pct} customers ({pct_of_customers:.1f}% of total customers)"
        )
        ...
        if customers_for_80_pct <= len(customer_data) * 0.2:
            ...
            result.append(
                "⚠️ More customers needed for 80% of revenue - Consider acquisition strategy"
            )
        else:
