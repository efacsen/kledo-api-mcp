...
must_haves:
  truths:
    - "revenue.py uses domain model for calculations (gross_sales not amount_after_tax)"
    - "invoices.py displays domain terminology in output"
    - "sales_analytics.py uses domain model for revenue aggregation"
    - "All existing tool functionality preserved"
    - "Tool descriptions updated to use clear terminology"
  artifacts:
    ...
    - path: "src/tools/invoices.py"
      ...
      contains: "gross_sales|net_sales"
...
- Report output uses "Gross Sales", "Net Sales", "PPN Collected"
...
```
  ...
  <done>
- Invoice list output uses "Gross Sales", "Net Sales" terminology
...
- Report headers use "Net Sales", "Gross Sales"
...
- Report outputs use "Gross Sales", "Net Sales", "PPN Collected"
