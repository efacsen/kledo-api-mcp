...
<task type="auto">
  ...
  <files>requirements.txt, src/routing/__init__.py</files>
  <action>
1. Add rapidfuzz>=3.14.0 to requirements.txt (after python-dateutil, before Development dependencies section)
...
4. Run pip install -r requirements.txt to ensure dependency is available
...
1. Import SYNONYM_MAP from src.routing.synonyms to use as default candidates when candidates parameter is None
